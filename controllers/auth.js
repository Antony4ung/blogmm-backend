...
const resetPassword = async (req,res) =>{
  try {
    ...
    if(!passwordIsMatch){
      res.status(400).json({ message: "Password is not match and can't update user data" });
      return;
    }

    const hashedPassword = await hashPassword(newPassword)

    const status = await user.findOneAndUpdate({email:email},{password:hashedPassword},{new:true})
